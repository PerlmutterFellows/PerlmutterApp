  ...
  def toggle_attendance(user_id, event_id, attend, output_as_flash)
    ...
    if !status.blank?
      if attend
        if !status.attending?
          status.attending!
          output_as_flash ? flash.now.notice = I18n.t('events.attending_success_response') : text_output = I18n.t('events.attending_success_response')
        else
          output_as_flash ? flash.now.alert = I18n.t('events.attending_failed_response') : text_output = I18n.t('events.attending_failed_response')
        end
      else
        if !status.not_attending?
          status.not_attending!
          output_as_flash ? flash.now.notice = I18n.t('events.not_attending_success_response') : text_output = I18n.t('events.not_attending_success_response')
        else
          output_as_flash ? flash.now.alert = I18n.t('events.not_attending_failed_response') : text_output = I18n.t('events.not_attending_failed_response')
        end
      end
    else
      output_as_flash ? flash.now.alert = I18n.t('global.invalid_input') : text_output = I18n.t('global.invalid_input')
    end
